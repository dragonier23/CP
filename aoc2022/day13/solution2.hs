...
main = do
    ...
    let parsed = parse contents
        lower = solution parsed (Node [Node [Packets 2]]) + 1
        upper = solution parsed (Node [Node [Packets 6]]) + 2
        answer = lower * upper
    print answer -- parsed --
    hClose handle

parse :: String -> [Tree]
parse = map (parse' . init . tail) . filter (not . null) . lines
...
solution xs y = sum . map (\x -> solution' x y) $ xs
