...
main = do
    ...
    let parsedContents = parse contents
        ...
        Node (_, size) _ _ = fileTree
        spaceRequired = size - 40000000
        answer = head . dropWhile (<spaceRequired) . sort $ sizeList fileTree
...
sizeList :: FileTree -> [Int]
sizeList (Node (a, size) [] []) = [0]
sizeList (Node (a, size) [] _) = [size]
sizeList (Node (a, size) xs _) = size: concatMap sizeList xs
