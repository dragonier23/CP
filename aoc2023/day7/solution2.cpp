...
int compare(vector<string> firstHand, vector<string> secondHand){ //logic behind whether one is larger than the other
    ...
    for (int i = 0; i < 5; i++){ // we are going to iterate through the list
        ...
        if (hand1[i] == 'J') { jcount1++; } else if (count1 > max11){ max12 = max11; max11 = count1; max1 = hand1[i]; } else if (count1 >= max12 && hand1[i] != max1) { max12 = count1 ;} //bug here --> if its the first one and it has 2 in a full house, it wont be recorded
        if (hand2[i] == 'J') { jcount2++; } else if (count2 > max21){ max22 = max21; max21 = count2; max2 = hand2[i]; } else if (count2 >= max22 && hand2[i] != max2) { max22 = count2 ;} 
    }
    // in most normal cases, we should add the jcount to max11 / max21 --> will calculate the total no
    // but we need to consider the case in which the joker is the most common card --> then we should be adding 
    // to the 2nd most common card pile 
    max11 = (jcount1 == 5) ? 5 : max11 + jcount1; 
    max21 = (jcount2 == 5) ? 5 : max21 + jcount2; 
